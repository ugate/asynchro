...
/**
 * Management for multiple `async function`/`await` tasks that can be ran in **series/sequential** and/or **parallel/concurrent** order
...
// TODO : ESM use... export class Asynchro {
  ...
  /**
   * Constructor
   * @arg {Object} [result] the object used for storing results (omit to prevent capturing of results)
   * @arg {(Boolean|Object|String)} [throws] one of the following values (unless superseded, will be applied to all queued tasks):
   ...
   * @arg {Function} [log] `function(tagArray, object)` that will log process output (omit to prevent logging)
   * @arg {Function} [includeErrorMsgCheck]
   */
   ...
   * @arg {String} [name] the name given for the task where the result will be stored as a property of the {@link Asynchro#result} object
   * (omit to prevent results from being set from function return value - a name/ID will be generated and returned)
   * @arg {Function} fn the function to queue for asynchronicity
   * @arg {...*} args aguments that will be passed into the queued function
   ...
   * @arg {String} [name] the name given for the task that can be used in conjunction with {@link Asynchro#verify}
   * (no results will be set from the function's return value - omit will cause the name/ID will be generated and returned)
   * @arg {(Boolean|Object|String)} [throws] one of the following values (supersedes any `throws` parameters passed during construction):
   * - `true` to throw any errors and instantly stop any further execution
   * - `false` to catch any errors that are thrown
   * - `Object` an object containing the following properties:
   * - - `invert` true to catch errors when matches are made, false/omit to throw errors when matches are made
   ...
   * @arg {String} name either the name designated as the property name or the `Function.name` from the function used when calling `parallel` or `series`
   * @arg {Function} fn an `async function` that will accept a single object argument that contains the follwing properties:
   ...
   * @arg {Function} fn a _synchronous_ `function` that will accept a single argument that will be either set to a new {@link Asynchro} instance when
   ...
   * @arg {Object} result the object where the background results will be set using a property name that matches the _name_ passed into 
   ...
   * @arg {String} [delimiter] the delimter to use between messages
   ...
   * @arg {(Error|Function)} errorOrType either an `Error` or a `Function` construct to an Error
   * @arg {Boolean} [throwWhenTrue] true to actually throw the error when `errorOrType` is an actual `Error` and it is determined that the error should throw
  ...
  /**
   * @returns {Function[]} an immutable array of error type constructs that will be thrown when they are encountered during queue execution
  ...
  /**
   * @returns {Object} the result of all of the cumulative execution results that had designated names assigned
   */
  get result() {
  ...
  /**
   * @returns {Error[]} an immutable array of all the caught errors encountered during execution
   */
  get errors() {
  ...
  /**
   * @returns {Integer} the total number of tasks queued for execution
   */
  get count() {
  ...
  /**
   * @returns {Integer} the number of tasks queued that are _waiting_ for execution
   */
  get waiting() {
  ...
  /**
   * @returns {String} the execution status
   */
  get status() {
  ...
  /**
   * @returns {String} the status indicating that the queue is available for tasks to be added and is waiting to be ran
   */
  static get QUEUEING() {return 'QUEUEING'; }
  /**
   * @returns {String} the status indicating that the queue is sealed from adding new tasks and is waiting to complete
   */
  static get RUNNING() {return 'RUNNING'; }
  /**
   * @returns {String} the status indicating that the queue was ran, but failed to complete
   */
  static get FAILED() { return 'FAILED'; }
  /**
   * @returns {String} the status indicating that the queue has successfully ran to completion
   */
  static get SUCCEEDED() { return 'SUCCEEDED'; }
  /**
   * @returns {String} the status indicating that the queue has been stopped before completing
   */
  static get STOPPED() { return 'STOPPED'; }
  /**
   * @returns {String} the status indicating that the queue has been transferred to another queue before completing
   */
  static get TRANSFERRED() { return 'TRANSFERRED'; }
  /**
   * @returns {Function[]} the default system error types that will be used for the `system` error type
   */
  static get DEFAULT_SYSTEM_ERROR_TYPES() { return ERROR_TYPES_SYSTEM; }

  /**
  * Promisifies event emitter event(s)
  * @arg {EventEmitter} eventEmitter the event emitter that will be listened to
  * @arg {(Array|String)} events the event(s) that will be listened for- either an `Object[]`, each containing event
  * properties or a `String[]`/`String` indicating event name(s) (strings will use the passed `tko`)
  * @arg {String} [events[].name] the event name
  * @arg {Integer} [events[].tko=tko] the timeout delay in milliseconds to wait for the event before _rejecting_ or
  * _resolving_ the promise (set to zero for _unlimited_ timeout)
  * @arg {Boolean} [events[].resolveOnTimeout] true to _resolve_ when the `tko` timeout is reached, false to _reject_
  * @arg {Integer} [tko=60000] the timeout delay in milliseconds to wait for the event before _rejecting_ or
  * _resolving_ the promise (set to zero for _unlimited_ timeout or override using `events[].tko`)
  * @arg {Boolean} [implyError=true] true will automatically listen for an `error` event that will reject or resolve
  * and uses the passed `tko` for the timeout delay
  * @arg {Integer} [eventMax=1] the number of events that must be emitted before the promise is resolved
  * @arg {Integer} [eventErrorMax=1] the number of events that must be emitted that contain an `Error` as the first argument
  ...
  */
   ...
   * @arg {Object} obj the object that contains the function that will be promisfied
   * @arg {String} fname the name of the function property in the `obj`
   * @arg {String[]} [fparams] an array of parameter names for the given function that will be used as the property names on the resolved promise
   ...
   * @arg {Integer} delay the number of milliseconds to delay before resolving/rejecting
   * @arg {*} val the value to use when resolving/rejecting
   * @arg {*} rejectIt true to reject, false to resolve after the delay expires
 ...
 * @arg {Asynchro} asyi the `async` processor
 * @arg {Boolean} series true to run in **series** relative to other tasks, false to run in **parallel**
 * @arg {(Boolean|Object|String)} [throws] one of the following values (supersedes any `throws` parameters passed during construction):
 * - `true` to throw any errors and instantly stop any further execution
 * - `false` to catch any errors that are thrown
 * - `Object` an object containing the following properties:
 ...
 * @arg {Function} fn the function to queue for asynchronicity
 * @arg {*} args aguments that will be passed into the queued function
 * @arg {Error[]} [bgErrors] an array that will store caught errors from queued background functions. The queued task function will __not__
 ...
 * @arg {Object} trk the tracking object from {@link Asynchro}
 * @arg {Asynchro} [asyi] the {@link Asynchro} instance
 ...
 * @arg {Object} trk the _private_ tracking object from {@link Asynchro}
 * @arg {Asynchro} asyi the `Asynchro` instance
 * @arg {Object} itm the queued `async` item from {@link asynchroQueue}
 * @arg {Object[]} pends the pending parallel/concurrent items- each originating from {@link asynchroQueue}
 ...
 * @arg {Object} itm the queued `async` item from {@link asynchroQueue}
 * @arg {Object[]} pends the pending parallel/concurrent items- each originating from {@link asynchroQueue}
 * @arg {Promise[]} [backgrounds] where background promises are kept
 ...
 * @arg {Object} itm the queued `async` item from {@link asynchroQueue}
 * @arg {*} systemErrorTypes the {@link Asynchro#systemErrorTypes} that will be applied to the background function check
 * @arg {Error[]} errors the {@link Asynchro#errors} where caught errors will be added
 */
 ...
 * @arg {Object} it the object to define the item metadata on
 * @arg {Object} itm the item where metadata will be extracted from
 * @arg {Boolean} pendPromise true when a promise is pending/waiting
 * @arg {String} [name] instead of defining properties/values directly on `it` they will be set on a newly defined `it[name] = {}`
 */
 ...
 * @arg {Asynchro} asyi the {@link Asynchro} instance
 * @arg {String} [name] the name of the task that was ran
 * @arg {(Error|Object|String)} [errorOrMessage] either an `Error`, `{ message }` or message string that wil be appended to the overall messages
 * @arg {String} [operation] an operation name that ran the task, typically a function name
...
class ResultArg {
  ...
  /**
   * Constructor
   * @arg {String} name the name of the property in the {@link Asynchro} `result` object
   */
  constructor(name) {
 ...
 * @arg {Object} [opts] merge options
 * @arg {Boolean} [opts.ctyp] flag that ensures that source values are constrained to the same type as the destination values when present
 * @arg {Boolean} [opts.nou] flag that prevents merge of undefined values
 * @arg {Boolean} [opts.non] flag that prevents merge of null values
 * @arg {Boolean} [opts.deep] flag that indicates that any objects/arrays found will be cloned instead of referenced
 * @arg {(Object[] | String[])} [opts.exc] properties to exclude from the merge
 * @arg {(Object | String)} [opts.exc[]] either object describing a property or a property name
 * @arg {String} [opts.exc[].prop] the property name to exclude
 * @arg {Integer} [opts.exc[].depth=Infinity] the depth level to exclude
 * @arg {Integer} [depth=1] the initial object depth used for exclusions
